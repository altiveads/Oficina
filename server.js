...
export default {
  ...
  async onMessage(message, ws, room) {
    ...
    switch (data.type) {
      ...
      // ── Verificar si usuario existe (probe) ───────────
      case "check_user": {
      ...
      case "register": {
        ...
        const username = data.username.toLowerCase().trim();
        if (profiles[username]) {
          ws.send(JSON.stringify({ type: "auth_error", message: "Ese usuario ya existe" }));
          return;
        }
        const newProfile = {
          username,
      ...
      case "join": {
        ...
        for (const conn of room.getConnections()) {
          if (conn.id !== ws.id && conn.state?.username) players[conn.id] = conn.state;
        }
        const tasks   = await room.storage.get("tasks") || [];
        const chatLog = await room.storage.get("chat")  || [];
        ws.send(JSON.stringify({ type: "init", players, tasks, chat: chatLog.slice(-80) }));
      ...
      // ── Movimiento ────────────────────────────────────
      case "move": {
        const updated = { ...ws.state, x: data.x, y: data.y, zone: data.zone, activity: data.activity, status: data.status };
        ws.setState(updated);
        ...
        room.broadcast(JSON.stringify({ type: "player_move", id: ws.id, ...data }), [ws.id]);
        break;
      }
      ...
      case "task_toggle": {
        ...
        await room.storage.put("tasks", tasks);
        room.broadcast(JSON.stringify({ type: "task_toggle", taskId: data.taskId, done: data.done, doneBy: data.doneBy }), [ws.id]);
        break;
      }
      case "task_delete": {
